...
- :math:`X_t` is the state variable at time :math:`t`, it is a stochastic process taking values in :math:`\mathbb{R}^n`
...
where :math:`\Delta W_t` is a normally distributed random variable with mean :math:`0` and variance :math:`\Delta t`, representing an increment of the Wiener process.
...
Under this definition, the Euler-Maruyama method exhibits the Euler-Maruyama method has a strong convergence order of :math:`0.5`, meaning that the error decreases proportionally to :math:`\sqrt{\Delta t}` as the time step :math:`\Delta t` approaches zero.
...
Consequently, the Euler-Maruyama method is particularly effective for approximating expectations of functionals of the solution, which is often the primary interest in applications involving SDEs.
